...
// ---- Formato números --------------------------------------
const F = {
  // Punto decimal, coma miles — formato estándar del sistema
  num:  (v, d=2) => v == null ? '—' : Number(v).toLocaleString('en-US', { minimumFractionDigits: d, maximumFractionDigits: d }),
  int:  (v)      => v == null ? '—' : Number(v).toLocaleString('en-US'),
  // Abreviatura inteligente: K hasta 999K, M desde 1,000,000
  abr:  (v)      => {
    if (v == null) return '—';
    const n = Number(v);
  ...
  date: (v)      => v ? new Date(v+'T00:00:00').toLocaleDateString('en-US', { day:'2-digit', month:'short', year:'numeric' }) : '—',
  ts:   (v)      => v ? new Date(v).toLocaleString('en-US', { day:'2-digit', month:'short', hour:'2-digit', minute:'2-digit' }) : '—',
};
