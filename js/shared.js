...
function renderSidebar(paginaActiva) {
  ...
  const moduloActivo = sessionStorage.getItem('dcp_modulo') || 'costos_piscinas';
  const esAdmin     = rol === 'admin';
  const esContador  = rol === 'contador' || esAdmin;

  // Items del módulo activo
  const itemsCostosPiscinas = [
    { seccion: 'Principal' },
  ...
  const itemsPorModulo = {
    costos_piscinas: itemsCostosPiscinas,
  };

  const items = itemsPorModulo[moduloActivo] || itemsCostosPiscinas;

  // Módulos disponibles para el switcher
  const modulosHTML = MODULOS.map(m => `
    <button class="modulo-btn ${m.id === moduloActivo ? 'activo' : ''} ${!m.activo ? 'proximamente' : ''}"
      onclick="${m.activo ? `cambiarModulo('${m.id}')` : 'mostrarProximamente()'}"
      title="${m.activo ? m.label : m.label + ' — Próximamente'}">
      ${m.icon}
      <span>${m.label}</span>
      ${!m.activo ? '<span class="prox-badge">Próximo</span>' : ''}
    </button>`).join('');
  ...
  const html = `
    <aside class="sidebar">
      <div class="sidebar-logo">
        ...
        <div><div class="sidebar-sub">Plataforma LUKMAR</div></div>
      </div>

      <!-- Módulos -->
      <div class="modulos-switcher">
        ${modulosHTML}
      </div>
      ...
      <div class="sidebar-nav">
        ...
        <a class="nav-item ${paginaActiva === 'admin_usuarios' ? 'active' : ''}" href="admin_usuarios.html">
          ${iconUsers} Usuarios
        </a>` : ''}
      </div>
...
function cambiarModulo(moduloId) {
  ...
  // Redirigir al dashboard del módulo
...
async function cerrarSesion() {
  ...
  const sb = createClient(window._SB_URL, window._SB_KEY);
